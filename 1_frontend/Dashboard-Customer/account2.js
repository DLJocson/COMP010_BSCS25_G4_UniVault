...
function requireLogin() {
  ...
  if (!cif) {
    ...
    window.location.href = '/1_frontend/Registration-Customer/login.html'; // Corrected login page path
    return false;
  }
  return true;
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  fetch(`/api/customer/${cif}`)
    ...
    .then(data => {
      ...
      displayCardAccounts(data.accounts || []);
...
function displayCardAccounts(accounts) {
  // Only show Card accounts for this page
  const cardAccounts = accounts.filter(acc => acc.account_type && acc.account_type.toLowerCase().includes('card'));
  const cardInfoCard = document.querySelector('.account-info-card');
  cardInfoCard.innerHTML = '';
  if (cardAccounts.length === 0) {
    cardInfoCard.innerHTML = '<div>No card accounts found.</div>';
    return;
  }
  cardAccounts.forEach(acc => {
    ...
    div.innerHTML = `
      <div class="account-icon"><img src="/assets/feature2.png" alt="" style="width:64px;height:64px;" /></div>
    ...
    `;
    cardInfoCard.appendChild(div);
  });
  // Fill in account details (first card by default)
  const details = cardAccounts[0] || {};
  ...
  if (!details.nickname && !details.account_number) {
    console.warn('No card account details found:', details);
  }
}

...
document.addEventListener('DOMContentLoaded', function() {
  ...
  fetch(`/api/customer/${cif}`)
    ...
    .then(data => {
      ...
      displayInsuranceAccounts(data.accounts || []);
...
function displayInsuranceAccounts(accounts) {
  // Only show Insurance accounts for this page
  const insuranceAccounts = accounts.filter(acc => acc.account_type && acc.account_type.toLowerCase().includes('insurance'));
  const insuranceInfoCard = document.querySelector('.account-info-card');
  insuranceInfoCard.innerHTML = '';
  if (insuranceAccounts.length === 0) {
    insuranceInfoCard.innerHTML = '<div>No insurance accounts found.</div>';
    return;
  }
  insuranceAccounts.forEach(acc => {
    ...
    div.innerHTML = `
      <div class="account-icon"><img src="/assets/insurance.png" alt="" style="width:64px;height:64px;" /></div>
    ...
    `;
    insuranceInfoCard.appendChild(div);
  });
  // Fill in account details (first insurance account by default)
  const details = insuranceAccounts[0] || {};
  ...
  if (!details.nickname && !details.account_number) {
    console.warn('No insurance account details found:', details);
  }
}

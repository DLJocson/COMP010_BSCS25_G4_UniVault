...
document.addEventListener('DOMContentLoaded', function() {
  ...
  fetch(`/api/customer/${cif}`)
    ...
    .then(data => {
      ...
      displayLoanAccounts(data.accounts || []);
...
function displayLoanAccounts(accounts) {
  // Only show Loan accounts for this page
  const loanAccounts = accounts.filter(acc => acc.account_type && acc.account_type.toLowerCase().includes('loan'));
  const loanInfoCard = document.querySelector('.account-info-card');
  loanInfoCard.innerHTML = '';
  if (loanAccounts.length === 0) {
    loanInfoCard.innerHTML = '<div>No loan accounts found.</div>';
    return;
  }
  loanAccounts.forEach(acc => {
    ...
    div.innerHTML = `
      <div class="account-icon"><img src="/assets/loan.png" alt="" style="width:64px;height:64px;" /></div>
    ...
    `;
    loanInfoCard.appendChild(div);
  });
  // Fill in account details (first loan by default)
  const details = loanAccounts[0] || {};
  ...
  if (!details.nickname && !details.account_number) {
    console.warn('No loan account details found:', details);
  }
}
